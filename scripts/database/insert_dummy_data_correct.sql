...
DECLARE
    ...
    manager_uuid UUID;
BEGIN
    -- 기존 룩업 테이블 ID 확인 또는 생성
    -- property_types에서 아파트 ID 찾기
    ...
    -- 빌라/연립 ID 찾기
    SELECT id INTO villa_id FROM property_types WHERE name IN ('빌라/연립', '빌라', '연립') LIMIT 1;
    IF villa_id IS NULL THEN
        INSERT INTO property_types (name) VALUES ('빌라/연립') RETURNING id INTO villa_id;
    END IF;
    
    -- transaction_types에서 ID들 찾기
    ...
    -- property_statuses에서 ID 찾기
    SELECT id INTO available_id FROM property_statuses WHERE name IN ('거래가능', '판매중', 'available') LIMIT 1;
    IF available_id IS NULL THEN
        INSERT INTO property_statuses (name) VALUES ('거래가능') RETURNING id INTO available_id;
    END IF;
    
    -- 기본 관리자 사용자 찾기
    SELECT id INTO manager_uuid FROM users WHERE email = 'jenny@the-realty.co.kr' LIMIT 1;
    
    -- 사용자가 없으면 NULL 사용 (또는 기본 사용자 생성)
    IF manager_uuid IS NULL THEN
        -- 옵션 1: NULL 사용
        manager_uuid := NULL;
        
        -- 옵션 2: 테스트 사용자 생성 (주석 해제하여 사용)
        -- INSERT INTO users (email, name, role) 
        -- VALUES ('jenny@the-realty.co.kr', 'Jenny', 'admin') 
    ...
    INSERT INTO properties (
        ...
        maintenance_fee, parking, special_notes, resident, manager_id,
    ...
    SELECT * FROM (VALUES
        ...
        NULL, 20000, 49, 38, 162.08, 125.62, 
        ...
        manager_uuid, NOW(), NOW())
    ) AS new_properties(property_name, location, property_type_id, transaction_type_id, property_status_id,
        price, lease_price, supply_area_pyeong, private_area_pyeong,
        supply_area_sqm, private_area_sqm, floor_info, rooms_bathrooms, direction,
        maintenance_fee, parking, special_notes, resident, manager_id, created_at, updated_at)
    WHERE NOT EXISTS (
...
-- 성공 메시지
SELECT '✅ 정확한 스키마 기반 더미 매물 데이터 삽입 완료!' as message;